...
def make_df_bb(train_path, test_path, img_size):
	...
	for i, id_ in enumerate(test_ids):
		...
		img = cv2.resize(img, (img_size, img_size))
		X_test[i] = img
	return X_train, Y_train0, Y_train1, Y_train2, Y_train_contour, X_test, sizes_test
...
if __name__ == "__main__":
	...
	model = Unet(img_size)
	model.compile(optimizer='adam', loss=[bce_dice_loss,bce_dice_loss], loss_weights=[.7,.3], metrics=[mean_iou, 'acc'])
	...
	modelname="model_dsbowl2018_test5c_"+st+".h5"
	...
	filename='sub-dsbowl2018_test5c_twooutputs_'+st+'.csv'

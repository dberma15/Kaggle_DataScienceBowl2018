...
def Unet(img_size):
	...
	outputs_box = Conv2D(3, (1, 1), activation='sigmoid')(c9)
...
if __name__ == "__main__":
	...
	model.compile(optimizer='adam', loss=[bce_dice_loss, bce_dice_loss,'mean_squared_error'], metrics=[mean_iou, 'acc'])
	
	for train, test in kfold.split(X,y):
		xtr=X_train[train]
		xval=X_train[test]
		
		ytr_bbox=Y_train_bbox[train]
		yval_bbox=Y_train_bbox[test]
		ytr_cntr0=Y_train_contour[train]
		yval_cntr0=Y_train_contour[test]
		ytr0=Y_train0[train]
		yval0=Y_train0[test]
		pickle.dump([xtr,ytr0,ytr_cntr0,ytr_bbox],open("7bBoundingBoxTraining.pkl",'wb'))					
	
		model.fit(xtr,[ytr0,ytr_cntr0,ytr_bbox],batch_size=16, epochs=50, validation_data=(xval,[yval0,yval_cntr0,yval_bbox]))
	
	ts=time.time()
	st=datetime.datetime.fromtimestamp(ts).strftime("%Y%m%d %H%M%S")

	modelname="model_dsbowl2018_test7b_"+st+".h5"
	
	model.save(modelname)
	
	preds_test, preds_contour, preds_boxes = model.predict(X_test, verbose=1)
	pickle.dump([preds_test, preds_contour, preds_boxes,X_test],open("7bBoundingBoxPreds.pkl",'wb'))					
